	...
	for j in range(ai_step[1] - 1, ai_step[1] + 2):
		if 0 <= i <= 6 and 0 <= j <= 6:
			if not (i == ai_step[0] and j == ai_step[1]):
				if fields[(i,j)] == "red":
					red_graph.addEdge(ai_step, (i,j))
					red_graph.addEdge((i,j), ai_step)
color = "blue"					
...
while running:
	...
	if not blue_won and color == "red":
		update_values()
		best_move = shortest_path()
		if best_move:
			fields[best_move] = "red"
			...
			for j in range(best_move[1] - 1, best_move[1] + 2):
				if i >= 0 and j >= 0 and i <= 6 and j <= 6:
					if not (i == best_move[0] and j == best_move[1]):
						if fields[(i, j)] == "red":
							red_graph.addEdge(best_move, (i, j))
							red_graph.addEdge((i, j), best_move)
